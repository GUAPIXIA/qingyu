...
- **在线更新**：集成 `electron-updater` 双通道更新（自托管镜像源优先、GitHub Releases 兑底），设置页新增「软件更新」卡片（检查/下载进度/重启安装/镜像源配置）；NSIS 开启增量更新（blockMap）；推送 `v*` 标签自动构建并发版到 GitHub Releases；公告服务端新增 `/update` 静态托管目录用于镜像分发。
