...
- **会话预览**：对话选择页改为展示最后一条有效消息，列表信息与实际对话进度一致。
- **移动端排版**：收紧输入区、快捷操作、消息字体和间距，操作栏保持单行并提升小屏空间利用率。
- **身份展示**：修复部分进入会话后头像、名称未按当前对话刷新的问题。
- **长期记忆兼容**：支持读取结构化事实及其状态，避免桌面端长记忆升级后安卓端解析或展示异常。
