...
## [0.1.5] - 2026-08-20（配合 PC 0.11.28）

- **配对安全收紧**：手动连接必须填写 PC 端生成的一次性配对码，已知设备也不能仅凭设备指纹换取新令牌
- **实时连接安全**：WebSocket 令牌改由请求头传递，不再出现在 URL 中；公网地址强制使用 HTTPS/WSS
- **循环重连修复**：响应 PC 的应用层心跳，并加入真实 WebSocket 握手回归测试；修复旧安装包使用 query token 被新版 PC 拒绝后持续显示“重连中”
- **受保护媒体**：头像、图片与语音等静态资源请求统一携带当前设备凭证
- **角色对话入口修复**：角色详情强制完整展开、正文独立滚动并固定操作区，可直接创建角色会话进入聊天
