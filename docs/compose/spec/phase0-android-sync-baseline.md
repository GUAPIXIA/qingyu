...
2. lint 失败源是未跟踪 `.qa-poc`，不是阶段0文件。
3. PowerShell 读 package.json 中文编码失败 → 指纹 JSON 改用 Node 写入。
4. 10k fixture 用生成器提交 JSONL，避免手工维护。
5. ADR-010 标明 Keystore 实机未验证，进阶段 7/8 前必须补测。
