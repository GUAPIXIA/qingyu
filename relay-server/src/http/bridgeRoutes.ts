...
export async function bridgeRoutes(app: FastifyInstance, options: { tokens: AccessTokenService; hub: RelayHub; validate: (claims: RelayAccessClaims) => Promise<boolean>; snapshots: SnapshotService; commands: CommandQueue; metrics?: MetricsRegistry }): Promise<void> {
  app.all('/relay/v1/bridge/*', { preHandler: requireRelayAuth(options.tokens, 'android', options.validate) }, async (request, reply) => {
    ...
    if (!route) return reply.code(404).send({ error: { code: 'RESOURCE_NOT_FOUND', message: '内容不存在', retryable: false, requestId: request.id } })
    ...
    if (!pc) {
      ...
      return reply.code(503).send({ error: { code: 'PC_OFFLINE', message: '电脑当前离线', retryable: true, requestId: request.id } })
    }
    ...
    try {
      ...
      return reply.code(timeout ? 504 : 503).send({ error: { code: timeout ? 'RPC_TIMEOUT' : 'PC_OFFLINE', message: timeout ? '电脑响应超时' : '电脑当前离线', retryable: true, requestId: request.id } })
    }
  })
}
