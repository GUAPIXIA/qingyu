...
- **H-18 存储型 XSS 绕过修复**：标签重建正则属性分隔 `\s+` → `[\s/]+`（`<img/src=x/onerror=...>` 斜杠分隔写法此前原样通过）+ 回归测试
- **M-35**：4 处 `req.body` 解构加空保护（Express 4 未认证畸形 POST 不再悬挂连接）
- **M-36**：`/api/version` PUT 全字段校验后原子写入（不再先写后校验致配置不一致）
...
- 编辑部暗色美学：炭墨底色 + 琥珀金强调（轻语品牌色），宋体标题 + 等宽数字，网格噪点氛围背景
- Bento 统计条（全部/已发布/草稿/置顶）
- 版本管理面板拆双端卡片（PC + 安卓）
- 登录/列表/弹窗精致化：卡片 hover、加载动画、玻璃遮罩、表单聚焦态
- 401 自动回登录页；API 路径保持 `/api/` 形式（nginx sub_filter 兼容）
