...
import { stripAllThinking } from '../thoughtMarkup'
...
function parseFactProposal(raw: unknown): FactProposal | null {
  ...
  if (!['subject', 'predicate'].every((key) => typeof item[key] === 'string' && item[key].trim())) return null
  if (item.changeType === 'set' && (typeof item.value !== 'string' || !item.value.trim())) return null
  if (item.changeType === 'clear' && item.value !== undefined && typeof item.value !== 'string') return null
  const proposal: FactProposal = {
    subject: String(item.subject).trim(),
    predicate: String(item.predicate).trim(),
    // clear 按 subject + predicate + scope + entityId 匹配旧事实，不依赖 value。
    // 模型通常会输出空串或省略该字段，统一归一化为空串以维持 FactProposal 契约。
...
function parseFactProposals(section: string): FactProposal[] | null {
  ...
  try {
    ...
    const proposals = value
      .map(parseFactProposal)
      .filter((proposal): proposal is FactProposal => proposal !== null)
    // JSON 数组本身有效时逐条容错：保留好条目；若非空数组全部无效，仍报告解析失败。
    return value.length > 0 && proposals.length === 0 ? null : proposals
  } catch {
    return null
  }
}
