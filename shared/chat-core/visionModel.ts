 ...
 *
 * 本模块不读取任何 store：识图模型与回退 Profile 由调用方显式传入
 * （渲染层读 settings store，主进程/Bridge 读各自持有的设置快照）。
 */
...
/** 识图模型配置中参与解析的字段（VisionModelConfig 的结构子集） */
...
/** 回退连接档案中参与解析的字段（ActiveProfile 的结构子集） */
export interface VisionProfileFallback {
...
export function resolveVisionModel(
  messages: VisionCandidate[],
  vision: VisionModelSource | null | undefined,
  fallback: VisionProfileFallback | null | undefined,
): ResolvedVisionModel | null {
  if (!contextHasImages(messages)) return null
  if (!vision || !vision.model?.trim()) return null

  const fallbackProvider = (fallback?.provider ?? 'openai') as ProviderType

  return {
    provider: (vision.provider?.trim() as ProviderType) || fallbackProvider,
    model: vision.model.trim(),
    baseUrl: vision.baseUrl?.trim() || fallback?.baseUrl || '',
    apiKey: vision.apiKey?.trim() || fallback?.apiKey || '',
  }
}
