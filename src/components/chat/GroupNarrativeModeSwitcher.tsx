...
interface GroupNarrativeModeSwitcherProps {
  isStreaming: boolean
}

/** 群聊会话级叙事模式；与点名/轮询/自由发言是两项独立设置。 */
export function GroupNarrativeModeSwitcher({ isStreaming }: GroupNarrativeModeSwitcherProps) {
  ...
  const setSessionNarrativeMode = useGroupChatStore((state) => state.setSessionNarrativeMode)
  ...
  const changeMode = async (mode: NarrativeMode) => {
    ...
    setSavingMode(mode)
    try {
      await setSessionNarrativeMode(mode)
      ...
      logError('GroupNarrativeModeSwitcher:updateSession', error)
        ...
        {NARRATIVE_MODE_OPTIONS.map((option) => {
          ...
          return (
            ...
            >
              ...
              <span className="hidden xl:inline">{option.value === 'immersive' ? '代入' : '全局'}</span>
            </button>
          )
        })}
      </div>
