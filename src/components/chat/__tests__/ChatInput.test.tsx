...
describe('ChatInput', () => {
  ...
  describe('基础渲染', () => {
    it('渲染输入框和发送按钮', () => {
      const { getByPlaceholderText, getByTitle } = render(<ChatInput character={createCharacter()} />)
    ...
    it('未连接时显示配置提示', () => {
      setupStores(false)
      const { getByPlaceholderText } = render(<ChatInput character={createCharacter()} />)
      expect(getByPlaceholderText(/请先在设置中配置 API 连接/)).toBeTruthy()
    })

    it('disabled 时禁用输入框', () => {
      const { getByPlaceholderText } = render(<ChatInput character={createCharacter()} disabled />)
  ...
  describe('发送逻辑', () => {
    it('空文本时发送按钮禁用', () => {
      const { getByTitle } = render(<ChatInput character={createCharacter()} />)
    ...
    it('流式时显示停止按钮', () => {
      ...
      const { getByTitle, queryByTitle } = render(<ChatInput character={createCharacter()} />)
  ...
  describe('引用回复（BUG-06 回归）', () => {
    ...
    it('被引用消息 content 为空时不崩溃（BUG-06）', () => {
  ...
  describe('图片预览', () => {
    it('渲染图片并可删除', () => {
      ...
      const { getByTitle, queryByAltText } = render(<ChatInput character={createCharacter()} />)
      // 无图片时不显示预览区
  ...
  describe('快捷回复', () => {
    it('渲染快捷回复按钮', async () => {
      ...
      const { findByText } = render(<ChatInput character={createCharacter()} />)
  ...
  describe('命令补全', () => {
    it('输入 / 前缀显示命令建议下拉', async () => {
      const { getByPlaceholderText, findByText } = render(<ChatInput character={createCharacter()} />)
  ...
  describe('AI 辅助', () => {
    it('润色按钮仅在输入非空时显示', () => {
      const { queryByTitle, getByPlaceholderText } = render(<ChatInput character={createCharacter()} />)
