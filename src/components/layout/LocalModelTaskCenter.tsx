...
export function LocalModelTaskCenter() {
  ...
  return <aside className="fixed bottom-4 right-4 z-40 w-80 rounded-lg border border-tavern-border bg-tavern-bg shadow-xl" aria-label="本地模型任务中心">
    ...
    {open && <div className="border-t border-tavern-border-soft px-3 py-2 space-y-2 max-h-48 overflow-auto">{visible.map((task) => {
      const progress = task.totalBytes ? Math.round(task.downloadedBytes / task.totalBytes * 100) : 0
      return <div key={task.taskId} className="text-xs"><div className="flex justify-between"><span className="truncate">{task.modelId}@{task.version}</span><span>{task.state} · {progress}%</span></div><div className="h-1 mt-1 rounded bg-tavern-bg-hover"><div className="h-full rounded bg-tavern-accent" style={{ width: `${progress}%` }} /></div></div>
    })}</div>}
  </aside>
}
