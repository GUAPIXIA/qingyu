...
export function installPreviewMock(): void {
  ...
  w.api = {
    ...
    lorebook: { list, save: noop, delete: noop },
    ...
    ai: { chat: noop, cancelChat: noop, testConnection: async () => ({ success: false }), listModels: async () => ({ success: false }), countTokens: async () => 0, countMessagesTokens: async () => [], compressLorebook: async () => '', localizeLorebookKeywords: async () => ({ suggestions: [] }), getGenerationUsageProfile: async () => null, getGenerationDiagnostics: async () => ({ modelProfile: { outputLimit: 32768, contextLimit: 32768, reasoningMode: 'shared-unknown', source: 'fallback', confidence: 'low' }, gateProbe: null, usageBuckets: [], lastRequest: null, observationStore: { loaded: true, keys: 0, scannedRecords: 0, skippedLines: 0 } }), resetGenerationGateProbe: noop, onChunk: () => () => {}, onComplete: () => () => {}, onError: () => () => {}, onUsage: () => () => {} },
