...
export function installPreviewMock(): void {
  ...
  w.api = {
    ...
    character: { list, get: async () => null, save: noop, delete: noop, importPng: async () => ({ success: false, canceled: true }), importJson: async () => ({ success: false, canceled: true }), importBatch: async () => ({ success: false, canceled: true }), bindLorebook: noop, exportPng: noop, exportJson: noop, reloadAvatar: async () => ({ success: false, avatar: '' }), onImportProgress: () => () => {} },
    chat: { listSessions: list, createSession: async () => ({ id: 'preview', characterId: 'preview', title: '预览会话', createdAt: Date.now(), updatedAt: Date.now() }), deleteSession: noop, renameSession: noop, updateSession: async () => ({} as never), listMessages: list, saveMessage: noop, deleteMessage: noop, clearChat: noop, exportChat: async () => '', updateMemory: noop, toggleMemory: noop, setMemoryMode: noop, getStats: async () => null, getMemoryHistory: async () => ({ history: [], total: 0 }) },
    ...
    imageGen: { generate: async () => ({ success: false }), testConnection: async () => ({ success: false }) },
    ...
    group: { list, save: noop, delete: noop, listSessions: list, createSession: async () => ({ id: 'preview', groupId: 'preview', title: '预览', messageCount: 0, createdAt: Date.now(), updatedAt: Date.now() } as never), deleteSession: noop, renameSession: noop, listMessages: list, saveMessage: noop, saveMessagesBatch: noop, editMessage: noop, deleteMessage: noop, clearChat: noop, exportChat: async () => '', updateMemory: noop, toggleMemory: noop, setMemoryMode: noop, updateSession: noop },
    ...
    ai: { chat: noop, cancelChat: noop, testConnection: async () => ({ success: false }), listModels: async () => ({ success: false }), countTokens: async () => 0, countMessagesTokens: async () => [], onChunk: () => () => {}, onDone: () => () => {}, onError: () => () => {}, onUsage: () => () => {} },
