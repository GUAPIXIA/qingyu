...
export function installPreviewMock(): void {
  ...
  w.api = {
    ...
    ai: { chat: noop, cancelChat: noop, testConnection: async () => ({ success: false }), listModels: async () => ({ success: false }), countTokens: async () => 0, countMessagesTokens: async () => [], compressLorebook: async () => '', localizeLorebookKeywords: async () => ({ suggestions: [] }), onChunk: () => () => {}, onComplete: () => () => {}, onError: () => () => {}, onUsage: () => () => {} },
