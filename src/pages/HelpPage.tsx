...
const faqs: { q: string; a: string }[] = [
  ...
  {
    q: '如何备份和迁移数据？',
    a: '在设置 → 数据管理页中可导出/导入完整备份（含角色、对话、设置、世界书、预设）。也可以单独导出角色卡为 PNG/JSON 格式进行分享。',
  },
  {
    q: '角色卡版本兼容情况如何？',
                    ...
                    return (
                      <div key={idx} className="flex gap-3">
                        <div className="flex flex-col items-center">
                          <div className="w-7 h-7 rounded-full bg-tavern-accent-soft text-tavern-accent flex items-center justify-center font-bold text-xs shrink-0">
                            {idx + 1}
                          </div>
                          {idx < steps.length - 1 && (
                            <div className="w-px flex-1 bg-tavern-border-soft my-0.5" />
                          )}
                        </div>
                        <div className="flex gap-2.5 pb-3.5">
                          <div className="w-8 h-8 rounded-lg bg-tavern-bg-hover flex items-center justify-center shrink-0">
                    ...
                    return (
                      <div key={idx} className="p-3 rounded-lg bg-tavern-bg-soft border border-tavern-border-soft">
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="text-xs font-medium text-tavern-text">{f.title}</span>
                        </div>
                        <p className="text-[11px] text-tavern-text-muted leading-relaxed">{f.desc}</p>
                      </div>
                    )
                  })}
                </div>
